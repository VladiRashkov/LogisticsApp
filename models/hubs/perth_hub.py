...
class Perth(BaseHub):
    ...
    def load_truck_remove_packages(self, truck_free_space, start_destination, end_destination):
        ...
        if truck_free_space > 0:
            if start_destination == Destination.PER and end_destination == Destination.ASP:
            ...
            elif start_destination == Destination.PER and end_destination == Destination.DAR:
                for package in self.destination_DAR:
                    if package.weight <= truck_free_space:
                        truck_storage.append(package)
                        self.destination_DAR.remove(package)

            elif start_destination == Destination.PER and end_destination == Destination.ADL:
                for package in self.destination_ADL:
                    if package.weight <= truck_free_space:
                        truck_storage.append(package)
                        self.destination_ADL.remove(package)

            elif start_destination == Destination.PER and end_destination == Destination.MEL:
                for package in self.destination_MEL:
                    if package.weight <= truck_free_space:
                        truck_storage.append(package)
                        self.destination_MEL.remove(package)

            elif start_destination == Destination.PER and end_destination == Destination.SYD:
                for package in self.destination_SYD:
                    if package.weight <= truck_free_space:
                        truck_storage.append(package)
                        self.destination_SYD.remove(package)

            elif start_destination == Destination.PER and end_destination == Destination.BRI:
